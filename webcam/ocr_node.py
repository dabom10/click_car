...
GOOGLE_VISION_CRED_PATH = "/home/rokey/click_car/web/google_vision.json"   # GCV 서비스 계정 키 경로
...
class ParkingDetectionNode(Node):
    ...
    def _preprocess_plate(img_crop: np.ndarray) -> np.ndarray:
        ...
        # ★ 추가된 부분: PaddleOCR은 3채널(RGB/BGR) 이미지를 요구하므로 1채널을 3채널로 변환
        cropped_3c = cv2.cvtColor(cropped, cv2.COLOR_GRAY2BGR)
    ...
    def _upload(self, item: dict):
        ...
        if plate_crop is not None:
            ...
            preprocessed = self._preprocess_plate(unwarped) # 업스케일+CLAHE+Otsu+테두리 제거
    ...
    def _ocr_paddle(self, img: np.ndarray) -> str:
        ...
        try:
            ...
            if not isinstance(results, list):
                results = list(results)   # 제너레이터일 수 있으므로 리스트로 변환
