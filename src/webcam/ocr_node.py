...
NOTE_GAP_MS = 100   # 음표 사이 침묵 간격 (ms). 노트 지속 시간에 더해 대기해
                    # 일부 음표가 씹히는 현상을 방지한다. 음표 길이 자체는 변경하지 않는다.

# ──────────────────────────────────────────────
# [CHAPTER 2-B: 캐논 변주곡 D장조 하이라이트 멜로디]
# ──────────────────────────────────────────────
# 각 튜플: (주파수 Hz, 지속 시간 ms)  ← 노트 길이만 기록; 간격은 NOTE_GAP_MS 로 별도 추가
#
# D장조 음계:
#   D4=294  E4=330  F#4=370  G4=392  A4=440  B4=494
#   C#5=554 D5=587  E5=659   F#5=740 G5=784  A5=880
#
# 구성 (총 ≈29초 @ NOTE_GAP_MS=100):
#   Phase 1 — 느린 주제 제시 (4분음표=600ms × 8음):          ~5.6s
#   Phase 2 — 유명한 8분음표 주행 (8분음표=300ms × 32음):   ~12.8s
#   Phase 3 — 클라이맥스 해소 (혼합, 반음표·온음표 포함):   ~10.8s
CANON_NOTES = [
    # ── Phase 1: 느린 주제 (하강→상승, 4분음표) ──────────────────────────────
    (740, 600), (659, 600), (587, 600), (554, 600),   # F#5 E5 D5 C#5
    (494, 600), (440, 600), (494, 600), (554, 600),   # B4  A4  B4  C#5

    # ── Phase 2: 8분음표 주행 (캐논의 핵심 패턴) ─────────────────────────────
    # 하강 후 반전
    (587, 300), (554, 300), (494, 300), (440, 300),
    (392, 300), (370, 300), (330, 300), (370, 300),
    # 아치형 상승
    (392, 300), (440, 300), (494, 300), (554, 300),
    (587, 300), (554, 300), (494, 300), (554, 300),
    # 정점을 향한 클라이밍
    (587, 300), (659, 300), (740, 300), (659, 300),
    (587, 300), (554, 300), (494, 300), (440, 300),
    # 클라이맥스 질주
    (494, 300), (554, 300), (587, 300), (659, 300),
    (740, 300), (784, 300), (880, 300), (784, 300),

    # ── Phase 3: 해소 및 마무리 ───────────────────────────────────────────────
    (740, 600), (659, 600), (587, 600), (554, 600),   # F#5 E5 D5 C#5 (주제 재현)
    (494, 600), (440, 600), (392, 600), (294, 600),   # B4  A4  G4  D4 (하강 종지)
    (440, 2000),                                       # A4 반음표 (숨 고르기)
    (587, 3000),                                       # D5 온음표 (대단원)
]
...
class ParkingDetectionNode(Node):
    ...
    def _play_canon(self):
        '''
        캐논 변주곡 D장조 하이라이트(≈29초)를 AudioNoteVector 토픽으로 퍼블리시한다.

        각 음표를 개별 메시지로 전송하고,
        음표 지속 시간(dur_ms) + NOTE_GAP_MS 만큼 대기해 음표 사이에 여유를 준다.
        NOTE_GAP_MS 간격이 있어야 로봇 오디오 드라이버가 음표를 누락하지 않는다.
        threading.Event.wait()를 사용해 중단 신호를 즉각 감지한다.
        '''
        self.get_logger().info("[Audio] 캐논 변주곡 멜로디 시작")
        for freq, dur_ms in CANON_NOTES:
            ...
            msg.notes  = [
                AudioNote(
                    ...
                    max_runtime = Duration(
                        sec     = 0,
                        nanosec = dur_ms * 1_000_000   # ms → ns
                    )
                )
            ]
            self.audio_pub.publish(msg)

            # 음표 지속 시간 + 간격만큼 대기 (간격이 있어야 음표 누락 방지)
            self._audio_stop_event.wait(timeout=(dur_ms + NOTE_GAP_MS) / 1000.0)

        self.get_logger().info("[Audio] 캐논 변주곡 멜로디 종료")
    ...
    def _upload(self, item: dict):
        ...
        ── cctv_start 모드 ──────────────────────────────────────────────────────
          ...
          confirmed: cctv_detections/<cctv_case_key> 에 아래 3가지만 추가
                       amr_evidence_image, plate_image, plate_number
                     → webcam_detector 가 생성한 케이스에 AMR 증거를 merge 한다.
        ...
        # ════════════════════════════════════════════════════════════════════
        # [A] cctv_start 모드: confirmed 시에만 cctv_detections 에 3가지 삽입
        # ════════════════════════════════════════════════════════════════════
