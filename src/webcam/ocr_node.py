...
from paddleocr import PaddleOCR               # 번호판 OCR 엔진 (PaddleOCR, fallback용)
from google.cloud import vision               # Google Cloud Vision OCR (메인 OCR 엔진)
...
GOOGLE_VISION_CRED_PATH = "/home/rokey/Downloads/google_vision.json"   # GCV 서비스 계정 키 경로
                                                                            # ※ 없으면 환경변수 GOOGLE_APPLICATION_CREDENTIALS 사용
...
class ParkingDetectionNode(Node):
    ...
    def __init__(self):
        ...
        self.ocr_reader       = None   # PaddleOCR 인스턴스 (초기화 후 설정, fallback용)
        self.gcv_client        = None   # Google Cloud Vision 클라이언트 (메인 OCR)

        self._load_model()       # YOLO 모델 로드 및 워밍업
        self._init_firebase()    # Firebase 연결 및 DB 참조 생성
        self._init_ocr()         # PaddleOCR 초기화 (fallback)
        self._init_gcv()         # Google Cloud Vision 초기화 (메인)
    ...
    def _init_gcv(self):
        '''
        Google Cloud Vision API 클라이언트를 초기화한다.

        Google Cloud Vision은 Google Lens와 동일한 OCR 엔진으로,
        한국어 번호판 인식에서 EasyOCR·PaddleOCR 대비 월등한 정확도를 보인다.
        월 1,000회까지 무료이므로 시뮬레이션 용도에서는 사실상 비용이 발생하지 않는다.

        인증 방법 (둘 중 하나):
          1. GOOGLE_VISION_CRED_PATH 파일 경로를 직접 지정
          2. 환경변수 GOOGLE_APPLICATION_CREDENTIALS 자동 참조
        실패해도 노드는 계속 실행되며, OCR 시 gcv_client None 체크로 PaddleOCR로 fallback된다.
        '''
        import os
        try:
            os.environ.setdefault(
                "GOOGLE_APPLICATION_CREDENTIALS", GOOGLE_VISION_CRED_PATH
            )
            self.gcv_client = vision.ImageAnnotatorClient()   # API 클라이언트 생성
    ...
    def _update_tracking(self, frame: np.ndarray, validated_pairs: list):
        ...
        for track in visible_tracks:
            if not track.confirmed_uploaded and track.elapsed() >= PARKING_TIMEOUT_SEC:
    ...
    def _preprocess_plate(img_crop: np.ndarray) -> np.ndarray:
        '''
        최신 딥러닝 OCR(GCV, PaddleOCR)은 원본 컬러 이미지에서 가장 잘 동작합니다.
        과도한 이진화(Otsu)나 필터는 오히려 인식률을 급감시키므로, 단순 업스케일만 적용합니다.
        '''
        h, w = img_crop.shape[:2]
        if h == 0 or w == 0:   # 크롭 실패 방어
            return img_crop

        # 1. 약간의 업스케일만 적용 (작은 번호판의 픽셀 수 확보)
        # 3배는 너무 클 수 있으니 2배 정도로만 키워줍니다.
        scale = 2
        preprocessed = cv2.resize(img_crop, (w * scale, h * scale), interpolation=cv2.INTER_CUBIC)
    ...
    def _upload(self, item: dict):
        ...
        # ── OCR: 번호판 크롭 → unwarp → 전처리 → GCV (실패 시 PaddleOCR fallback) ──
        plate_number = "UNKNOWN"   # 인식 실패 기본값
        if plate_crop is not None:
            # unwarped     = self._unwarp_plate(plate_crop)   # 비스듬한 번호판 평탄화
            preprocessed = self._preprocess_plate(plate_crop) # 업스케일+CLAHE+Otsu+테두리 제거

            # # ── 1순위: Google Cloud Vision API ──
            # if self.gcv_client is not None:
            #     plate_number = self._ocr_gcv(preprocessed)

            # ── 2순위: PaddleOCR (GCV 미연결 또는 인식 실패 시 fallback) ──
            if plate_number == "UNKNOWN" and self.ocr_reader is not None:
                plate_number = self._ocr_paddle(preprocessed)
    ...
    def _ocr_gcv(self, img: np.ndarray) -> str:
        '''
        Google Cloud Vision API로 번호판 이미지를 인식하고 텍스트를 반환한다.

        처리 흐름:
          1. OpenCV 이미지 → JPEG 바이트 → vision.Image 객체로 변환
          2. document_text_detection() 호출
             - text_detection()보다 밀집된 텍스트(번호판)에 더 적합한 모드
          3. full_text_annotation.text 에서 전체 인식 결과 추출
          4. 공백·개행 제거 후 반환
          5. 예외 발생 또는 결과 없으면 "UNKNOWN" 반환 (PaddleOCR fallback 유도)
        '''
        try:
            _, buf     = cv2.imencode(".jpg", img, [cv2.IMWRITE_JPEG_QUALITY, 95])   # GCV는 고품질 권장
            gcv_image  = vision.Image(content=buf.tobytes())                          # vision.Image 객체 생성
            response   = self.gcv_client.document_text_detection(image=gcv_image)    # 밀집 텍스트 인식 모드
            annotation = response.full_text_annotation

            if response.error.message:   # API 레벨 오류 확인
                self.get_logger().warn(f"GCV API 오류: {response.error.message}")
                return "UNKNOWN"

            text = annotation.text.replace(" ", "").replace("\n", "")   # 공백·개행 제거
    ...
    def _ocr_paddle(self, img: np.ndarray) -> str:
        ...
        Google Cloud Vision 미연결 또는 인식 실패 시 fallback으로 호출된다.

        반환값 구조 처리:
          PaddleOCR 최신 버전(PaddleX 기반)은 predict() 메서드를 사용하며,
          결과 객체가 get_res() 메서드를 가진 경우와 리스트 형태 두 가지를 모두 처리한다.
        ...
        try:
            ...
            # ★ 핵심 추가: 언어 힌트를 한국어('ko')로 강제 설정
            image_context = vision.ImageContext(
                language_hints=["ko"]
            )

            # API 호출 시 image_context도 함께 전달
