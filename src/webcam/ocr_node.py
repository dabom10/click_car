...
# ★ 박자를 2배로 늘린 이유
# Create3 AudioNoteVector 내부 버퍼는 약 50음표 처리가 한계.
# 원본 62음표 중 연속 8분음표 32개 구간(Phase 2-A~3-A)에서 버퍼 오버플로로
# 일부 음표가 드랍되어 "음이 무시되는" 현상이 발생했다.
# 박자를 2배 늘리면: 기존 8분음표→600ms, 4분음표→1200ms
# Phase 2-B~3-B(38음표)만으로 31.2초를 채울 수 있어 Phase 1·2-A를 제거했다.
# 결과: 음표 수 62개→38개로 감소, 버퍼 오버플로 해소, 재생시간 31.2초 확보.
_Q2 = 1_200_000_000   # 기존 4분음표(600ms)의 2배
_E2 =   600_000_000   # 기존 8분음표(300ms)의 2배
_H2 = 2_400_000_000   # 기존 2분음표(1200ms)의 2배
_W2 = 4_800_000_000   # 기존 온음표(2400ms)의 2배
CANON_NOTES = [
    # ── Phase 2-B: 아치형 상승 (16음표 × 600ms = 9.6초) ──────────────────────
    (494, _E2), (554, _E2), (587, _E2), (659, _E2),   # B4  C#5 D5  E5
    (587, _E2), (554, _E2), (494, _E2), (554, _E2),   # D5  C#5 B4  C#5
    (587, _E2), (659, _E2), (740, _E2), (659, _E2),   # D5  E5  F#5 E5
    (587, _E2), (554, _E2), (494, _E2), (440, _E2),   # D5  C#5 B4  A4

    # ── Phase 3-A: 클라이맥스 질주 (16음표 × 600ms = 9.6초) ─────────────────
    (494, _E2), (554, _E2), (587, _E2), (659, _E2),   # B4  C#5 D5  E5
    (740, _E2), (784, _E2), (880, _E2), (784, _E2),   # F#5 G5  A5  G5
    (740, _E2), (659, _E2), (587, _E2), (554, _E2),   # F#5 E5  D5  C#5
    (494, _E2), (440, _E2), (392, _E2), (330, _E2),   # B4  A4  G4  E4

    # ── Phase 3-B: 주제 재현 + 종지 (4×1200ms + 2400ms + 4800ms = 12.0초) ───
    (370, _Q2), (440, _Q2), (494, _Q2), (587, _Q2),   # F#4 A4  B4  D5
    (740, _H2),                                         # F#5 (정점)
    (587, _W2),                                         # D5  (종지)
]
# 총 재생시간: 9.6 + 9.6 + 12.0 = 31.2초
del _Q2, _E2, _H2, _W2
