...
# ── 불법주정차 단속 구역 (map 좌표계, axis-aligned bbox) ──
# 이 구역 내에 car 좌표가 포함된 경우에만 publish
# 형식: (x_min, y_min, x_max, y_max)
# 구역 밖 차량은 탐지는 하되 publish 하지 않음
ILLEGAL_ZONES = [
    (-1.91, 0.46, -0.80, 1.60),   # 구역 1
    (-1.24, -5.00, 0.504, -4.30), # 구역 2
]
...
# ── 불법주정차 구역 (map 좌표계, 직사각형 bbox) ──────
# 꼭짓점 4쌍에서 추출한 x/y 범위.
# 추정 좌표가 이 구역 안에 있을 때만 publish.
# 구역 추가/수정: 아래 리스트에 (x_min, x_max, y_min, y_max) 튜플 추가.
ILLEGAL_ZONES = [
...
class ParkingDetectionNode(Node):
    def __init__(self):
        ...
        # ── TF2: odom → map 좌표 변환용 ──
        # SLAM localization 실행 중이면 /robot3/tf 에 map→odom TF 발행됨
        # TransformListener는 기본적으로 /tf를 구독하므로
        # main()에서 remapping으로 /tf → /robot3/tf 연결
    ...
    def _in_illegal_zone(x: float, y: float) -> bool:
        ...
        axis-aligned bbox: x_min ≤ x ≤ x_max, y_min ≤ y ≤ y_max
        """
        for x_min, y_min, x_max, y_max in ILLEGAL_ZONES:
